...
Set `Version:` and `Stable tag:` to the current value in root [`package.json`](../package.json). At release time, `/do-usecase-release` bumps the monorepo version first, then `composer run sync:version -- --changed-since <prev-tag>` updates changed plugins (and WP compatibility on all plugins).
